...
import { verifyFulfillmentFailureRequest } from "../../../lib/palangDesktop/queue";
...
export default async function handler(
  ...
  try {
    ...
    try {
      callback = JSON.parse(rawBody.toString("utf8"));
    } catch {
      sendNonRetryableError(res, "invalid_payload", "Payload is invalid");
      return;
    }
    const message = parseFulfillmentFailureCallback(callback);
    ...
    res.status(200).json({ recovered: true, email: result.email });
  } catch (error) {
    if (
    ...
    if (error instanceof ServerConfigurationError) {
      sendApiError(res, 503, "not_configured", "Recovery is not configured");
      return;
    }
    sendApiError(
      res,
      ...
      "Fulfillment recovery is temporarily unavailable",
    );
  }
}
